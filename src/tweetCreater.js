...
const createTemplate = (tweet, index) => {
  const { 
    user_icon,
    user_name,
    user_id,
...
const createTweets = json => {
  return json.map((tweet, index) => {
    const div = document.createElement('div')
    div.innerHTML = createTemplate(tweet, index)
    return div
  })
}
...
export const tweetRenderer = () => renderTweets(createTweets(tweets))